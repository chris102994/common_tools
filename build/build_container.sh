...
-t, --testc   - Run the Docker tests: (Default=false) Test's the container you just built and update the results on github.
	"
	exit 1
}
...
while getopts 'be:hprgt' 'option'; do
	case "${option}" in
		'b') BUILD='true' ;;
		'e') ENV="$ENV ${OPTARG}" ;;
		'h') usage ;;
...
Test Container & push results	(-t, --testc): ${TEST_CONTAINER}
#####################
"
...
# Test the container and push results to github

...
if [ $? -eq 0 ]; then
    GIT_VERSION=$(echo "${GIT_VERSION}" |  grep -Eo '[0-9]*\.[0-9]*\.[0-9]*' )
...
GIT_MESSAGE="$(git show -s --format=%s | tr '[:lower:]' '[:upper:]')"
...
GIT_BRANCH="$(git rev-parse --abbrev-ref HEAD | sed "s#HEAD#${TRAVIS_BRANCH:-DETACHED}#i")"
